...
**Not yet built:**
- Anyone with the site URL can read all scouting data and add/edit entries — the publishable key ships in the client and the RLS policies allow it, deliberately, so scouts need no login. Deletes are blocked. To restrict writes to teammates, enable Supabase Auth and switch the policies from `to anon` to `to authenticated`.
